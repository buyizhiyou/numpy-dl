...
class Convolution(Layer):
    ...
    def connect_to(self, prev_layer=None):
        ...
        # filters
        self.W = self.init((self.nb_filter, pre_nb_filter,
            ...
            for y in np.arange(self.nb_filter):
                for h in np.arange(new_img_h):
                    for w in np.arange(new_img_w):
                        ...
                        patch = input[x, :, h_shift: h_shift +
                                      filter_h, w_shift: w_shift + filter_w]
                        outputs[x, y, h, w] = np.sum(
                            patch * self.W[y]) + self.b[y]
            ...
            for t in np.arange(input_depth):
                for h in np.arange(filter_h):
                    for w in np.arange(filter_w):
                        ...
                        self.dW[r, t, h, w] = np.sum(
                            input_window * delta_window) / nb_batch
