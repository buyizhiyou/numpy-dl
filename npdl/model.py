...
class Model(object):
    ...
    def fit(self, X, Y, max_iter=100, batch_size=64, shuffle=True,
        ...
        # prepare data
        train_X = X.astype(get_dtype()) if np.issubdtype(
            np.float64, X.dtype) else X
        train_Y = Y.astype(get_dtype()) if np.issubdtype(
            np.float64, Y.dtype) else Y
        ...
        while iter_idx < max_iter:
            ...
            for b in range(train_Y.shape[0] // batch_size):
                ...
                # update parameters
                print("epoch:{},batch:{},update params...".format(iter_idx, b))
